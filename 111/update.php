...
if (isset($_GET['id'])) {
    if (!empty($_POST)) {
        ...
        // Update the record
        $stmt = $pdo->prepare('UPDATE contacts SET id = ?, name = ?, email = ?, phone = ?, course = ?, created = ? WHERE id = ?');
        $stmt->execute([$id, $name, $email, $phone, $course, $created, $_GET['id']]);
...
<div class="content update">
    ...
    <form action="update.php?id=<?=$contact['id']?>" method="post">
        ...
        <input type="text" name="course" placeholder="Employee" value="<?=$contact['course']?>" id="title">
